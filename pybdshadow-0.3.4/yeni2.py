...
def update_plot(*args):
    ...
    sun_vector_x = -np.cos(sun_azimuth) * np.cos(sun_elevation)
    sun_vector_y = -np.sin(sun_azimuth) * np.cos(sun_elevation)
    sun_vector_z = -np.sin(sun_elevation)
    shadow_length = float(object_height) / math.tan(sun_elevation)
...
object_height = input("Enter object height: ")
...
azimuth_scale = tk.Scale(root, label="Sun Azimuth", from_=-180, to=180, orient="horizontal")
elevation_scale = tk.Scale(root, label="Sun Elevation", from_=1, to=90, orient="horizontal")
